...
public class NamespaceFactoryImpl implements NamespaceFactory {
    ...
    public MutablePackageFragmentDescriptor createPackageFragmentIfNeeded(@NotNull JetFile file, @NotNull RedeclarationHandler handler) {
        ...
        for (JetSimpleNameExpression nameExpression : allNamespaceNames) {
            ...
            createPackageFragment(parentFqName, nameExpression, handler);

            trace.record(RESOLUTION_SCOPE, nameExpression, module.getPackage(parentFqName).getMemberScope());
        }

        FqName fqName = namespaceHeader.getFqName();
        storeBindingForFile(file, fqName);
        return createPackageFragment(fqName, namespaceHeader.getLastPartExpression(), handler);
    }
    ...
    public PackageFragmentDescriptor createNamespaceDescriptorPathIfNeeded(@NotNull FqName fqName) {
        for (FqName pathElement : fqName.path()) {
            if (pathElement.isRoot()) {
                createPackageFragment(FqName.ROOT, null, RedeclarationHandler.DO_NOTHING);
            }
            else {
                createPackageFragment(pathElement, null, RedeclarationHandler.DO_NOTHING);
            }
    ...
    @NotNull
    private MutablePackageFragmentDescriptor createPackageFragment(
    ...
    private void storeBindingForFile(@Nullable JetFile file, @NotNull FqName fqName) {
        if (file != null) {
